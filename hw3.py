# Задание №2 Задание к 3 уроку ниже
...
vacancies_table.to_csv('vacancies.csv', sep=';', encoding='utf-8')

client = MongoClient('127.0.0.1', 27017)
db = client['vacancies']
vac = db.vac

for i in range(0, len(vacancies_table)):
    vac.insert_one(vacancies_table.loc[i].to_dict())
    vac.update_one({'link': vacancies_table.loc[i].to_dict()['link']}, {
                  '$set': vacancies_table.loc[i].to_dict()}, upsert=True)


min_wage = int(input('Введите мин. зарплату: '))

for item in vac.find({ '$and': [{ 'wage_currency': 'руб.' }, {'wage_from': { '$gt': min_wage }}]}):
    pprint(item)
